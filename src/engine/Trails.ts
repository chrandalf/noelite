...
const POINTS = 40
const TIPS = [new THREE.Vector3(-3.5, -0.05, 2.9), new THREE.Vector3(3.5, -0.05, 2.9)]
...
export class Trails {
  ...
  update(dt: number, craftPos: THREE.Vector3, quat: THREE.Quaternion, strength: number, show: boolean): void {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      this.tmp.copy(TIPS[i]).applyQuaternion(quat).add(craftPos)
      p[0] = this.tmp.x; p[1] = this.tmp.y; p[2] = this.tmp.z
      const k = Math.min(1, Math.max(0, strength))
      c[0] = 0.9 * k; c[1] = 0.95 * k; c[2] = 1.0 * k
      // Fade along the ribbon.
      for (let j = 1; j < POINTS; j++) { const f = 1 - j / POINTS; c[j * 3] *= 0.97; c[j * 3 + 1] *= 0.97; c[j * 3 + 2] *= 0.97; void f }
