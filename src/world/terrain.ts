...
export function groundRadius(d: THREE.Vector3, seed: PlanetSeed): number {
  return PLANET_RADIUS + height(d, seed)
}
...
function at(d: THREE.Vector3, du: number, dv: number, seed: PlanetSeed, out: THREE.Vector3): THREE.Vector3 {
...
export function surfaceNormal(d: THREE.Vector3, seed: PlanetSeed, out = new THREE.Vector3()): THREE.Vector3 {
  ax.set(Math.abs(d.x) < 0.9 ? 1 : 0, Math.abs(d.x) < 0.9 ? 0 : 1, 0)
  t1.crossVectors(ax, d).normalize()
  t2.crossVectors(d, t1)
  const e = 1 / PLANET_RADIUS
...
export function slopeDeg(d: THREE.Vector3, seed: PlanetSeed): number {
...
export function findLandable(guess: THREE.Vector3, seed: PlanetSeed, maxSlope = 8, out = new THREE.Vector3()): THREE.Vector3 {
